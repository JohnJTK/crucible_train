defmodule CrucibleTrain.Utils.PRNG.SeedSequence do
  @moduledoc """
  ...
  defp mix_entropy(entropy, pool_size) do
    ...
    {pool, hash_const} = hash_entropy_into_pool(pool, entropy, pool_size)
    ...
    {pool, _hash_const} = mix_remaining_entropy(pool, entropy, pool_size, hash_const)

    pool
  end

  defp hash_entropy_into_pool(pool, entropy, pool_size) do
  ...
  defp mix_pool_bits(pool, pool_size, hash_const) do
    Enum.reduce(0..(pool_size - 1), {pool, hash_const}, fn i_src, acc ->
      mix_pool_row(acc, i_src, pool_size)
    end)
  end

  defp mix_pool_row({pool, hash_const}, i_src, pool_size) do
  ...
  defp mix_remaining_entropy(pool, entropy, pool_size, hash_const) do
    Enum.reduce(pool_size..(length(entropy) - 1)//1, {pool, hash_const}, fn i_src,
                                                                            {pool, hash_const} ->
      mix_remaining_row(pool, entropy, i_src, pool_size, hash_const)
    end)
  end

  defp mix_remaining_row(pool, entropy, i_src, pool_size, hash_const) do
    Enum.reduce(0..(pool_size - 1), {pool, hash_const}, fn i_dst, {pool, hash_const} ->
