...
defmodule CrucibleTrain.Supervised.DatasetFromList do
  @moduledoc """
  ...
  @behaviour CrucibleTrain.Supervised.Dataset
...
defmodule CrucibleTrain.Supervised.DatasetFromSamples do
  @moduledoc """
  ...
  - Shuffles samples on `set_epoch/2` with a deterministic seed
  ...
  @type shuffle_mode :: :erlang | :pcg64
  ...
  defstruct [:samples, :shuffled_samples, :batch_size, :datum_builder, shuffle: :erlang]
  ...
  - `opts` - Optional keyword list
    - `:shuffle` - `:erlang` (default) or `:pcg64` for NumPy-compatible shuffling
  ...
  """
  @spec new([map()], pos_integer(), datum_builder(), keyword()) :: t()
  def new(samples, batch_size, datum_builder, opts \\ [])
      when is_list(samples) and batch_size > 0 and is_function(datum_builder, 1) do
    shuffle = Keyword.get(opts, :shuffle, :erlang)
  ...
  @impl true
  @spec set_epoch(t(), non_neg_integer()) :: t()
  def set_epoch(%__MODULE__{samples: samples, shuffle: :pcg64} = dataset, seed) do
  ...
  def set_epoch(%__MODULE__{samples: samples} = dataset, seed) do
    %{dataset | shuffled_samples: erlang_shuffle(samples, seed)}
  end

  defp erlang_shuffle(samples, seed) do
    samples
    |> Enum.with_index()
    |> Enum.sort_by(fn {_item, idx} -> :erlang.phash2({seed, idx}) end)
    |> Enum.map(fn {item, _idx} -> item end)
  end
end

defmodule CrucibleTrain.Supervised.DatasetFromSamplesFlatMap do
  @moduledoc """
  ...
  defstruct [:samples, :shuffled_samples, :batch_size, :flatmap_fn, shuffle: :erlang]

  @spec new([map()], pos_integer(), flatmap_fn(), keyword()) :: t()
  def new(samples, batch_size, flatmap_fn, opts \\ [])
