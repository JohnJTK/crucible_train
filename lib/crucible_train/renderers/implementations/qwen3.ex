defmodule CrucibleTrain.Renderers.Implementations.Qwen3 do
  @moduledoc """
...
defmodule CrucibleTrain.Renderers.Implementations.Qwen3DisableThinking do
  @moduledoc """
  ...
  alias CrucibleTrain.Renderers.{Implementations.Qwen3, Renderer}
  ...
  @spec build_generation_prompt(
          [CrucibleTrain.Renderers.Types.Message.t()],
          String.t(),
          String.t() | nil,
          map()
        ) ::
          {CrucibleTrain.Types.ModelInput.t(), map()}
...
defmodule CrucibleTrain.Renderers.Implementations.Qwen3Instruct do
  @moduledoc """
  ...
  alias CrucibleTrain.Renderers.{Helpers, ToolCalls, Types}
  alias CrucibleTrain.Renderers.Implementations.Qwen3
  alias CrucibleTrain.Types.EncodedTextChunk
  ...
  def render_message(idx, message, _is_last, %{tokenizer: tokenizer} = state) do
    ...
    ac_content =
      if is_list(message.tool_calls) and message.tool_calls != [] do
        content <> ToolCalls.encode_qwen3(message.tool_calls)
      else
        content
      end
